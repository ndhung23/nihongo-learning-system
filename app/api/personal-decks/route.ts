...
const CreatePersonalDeckSchema = z.object({
  ...
  accessMode: z.enum(["public", "private", "password", "invite"]).default("private"),
...
export async function GET() {
  try {
    ...
    const ownershipFilter = session.roles.includes("admin") ? {} : { $or: [{ ownerId: session.userId }, { allowedUserIds: session.userId }] };
    const decks = await DeckModel.find({ ...ownershipFilter, sourceType: "user", tags: "personal" })
      .select("title slug description stats accessMode allowedUserIds ownerId visibility updatedAt").sort({ updatedAt: -1 }).lean();
...
export async function POST(request: NextRequest) {
  try {
    ...
    const deck = await DeckModel.create({
      title: payload.title, slug, description: payload.description || "", level: "custom",
      sourceType: "user", ownerId: session.userId, visibility: payload.accessMode === "public" ? "public" : "private", status: "draft",
      accessMode: payload.accessMode, accessPasswordHash, allowedUserIds,
      tags: ["personal"], stats: { vocabularyCount: 0, learnerCount: 0 },
    });
    return NextResponse.json({ data: { _id: String(deck._id), title: deck.title, description: deck.description, vocabularyCount: 0, accessMode: payload.accessMode, invitedEmails: payload.invitedEmails } }, { status: 201 });
  } catch (error) {
