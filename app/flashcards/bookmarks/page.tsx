...
import { FiBookmark, FiCompass, FiSearch, FiTrash2 } from "react-icons/fi";
      ...
      {bookmarks.length === 0 ? (
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
          {filteredBookmarks.map((bookmark) => (
            <article className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-xl shadow-slate-900/[0.04] transition hover:-translate-y-1 hover:border-teal-300 hover:shadow-2xl hover:shadow-teal-500/10" key={bookmark.key}>
              ...
              {bookmark.deckId ? (
                <Link className="mt-5 flex h-11 items-center justify-center rounded-2xl bg-slate-950 font-black text-white transition hover:-translate-y-0.5 hover:bg-teal-700" href={`/flashcards/study?mode=flashcard&deckId=${bookmark.deckId}`}>
                  Học lại khóa này
                </Link>
              ) : null}
            </article>
