...
import { FiArrowLeft, FiBookOpen, FiCheckSquare, FiEdit3, FiFolder, FiPlay, FiPlus, FiSquare, FiX } from "react-icons/fi";
import { FuriganaText } from "../components/FuriganaText";
import { EmailInvitePicker } from "../components/EmailInvitePicker";

type AccessMode = "public" | "private" | "password" | "invite";
type PersonalDeck = { _id: string; title: string; description?: string; vocabularyCount: number; accessMode?: AccessMode; invitedEmails?: string[]; canEdit?: boolean; canAdd?: boolean };
...
export function PersonalDecksClient() {
  ...
  useEffect(() => {
    ...
    let active = true;
    void fetch("/api/personal-decks", { cache: "no-store" }).then((response) => response.json()).then((payload: { data?: PersonalDeck[] }) => {
  ...
  async function createDeck(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(editingId ? `/api/personal-decks/${editingId}` : "/api/personal-decks", { method: editingId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title, description, accessMode, password: password || undefined, invitedEmails }) });
  ...
  function openCreate() { setEditingId(""); setTitle(""); setDescription(""); setAccessMode("private"); setPassword(""); setInvitedEmails([]); setCreateOpen(true); }
  function openEdit(deck: PersonalDeck) { setEditingId(deck._id); setTitle(deck.title); setDescription(deck.description || ""); setAccessMode(deck.accessMode || "private"); setPassword(""); setInvitedEmails(deck.invitedEmails || []); setCreateOpen(true); }
  ...
  return <div className="mx-auto max-w-[1500px] px-4 py-8 sm:px-6 lg:px-10">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.28em] text-rose-600">Cá nhân</p><h1 className="mt-3 text-4xl font-black tracking-tight text-slate-950 sm:text-5xl">{deckId ? selectedDeck?.title || "Bộ từ vựng" : "Bộ từ của tôi"}</h1><p className="mt-3 max-w-2xl text-slate-500">{deckId ? selectedDeck?.description || "Thêm và luyện các từ trong bộ này." : "Tạo từng bộ theo chủ đề, sau đó mở bộ để thêm từ vựng."}</p></div>
      <div className="flex flex-wrap gap-3">{deckId ? <><Link className="inline-flex h-12 items-center gap-2 rounded-2xl border border-slate-200 bg-white px-5 font-black text-slate-700" href="/flashcards/my-vocabulary"><FiArrowLeft /> Các bộ từ</Link>{selectedDeck?.canEdit && <button className="inline-flex h-12 items-center gap-2 rounded-2xl border border-teal-200 bg-teal-50 px-5 font-black text-teal-800" onClick={() => openEdit(selectedDeck)} type="button"><FiEdit3 /> Sửa bộ</button>}{items.length > 0 && <Link className="inline-flex h-12 items-center gap-2 rounded-2xl bg-teal-600 px-5 font-black text-white" href={`/flashcards/study?scope=mine&mode=flashcard&deckId=${encodeURIComponent(deckId)}`} onClick={() => preparePractice(selectedIds.length ? selectedIds : items.map((item) => item._id))}><FiPlay /> {selectedIds.length ? `Luyện ${selectedIds.length} từ` : "Luyện cả bộ"}</Link>}{selectedDeck?.canEdit && <Link className="inline-flex h-12 items-center gap-2 rounded-2xl bg-rose-600 px-5 font-black text-white" href={`/flashcards/add?deckId=${encodeURIComponent(deckId)}`}><FiPlus /> Thêm từ</Link>}</> : <button className="inline-flex h-12 items-center gap-2 rounded-2xl bg-rose-600 px-5 font-black text-white" onClick={openCreate} type="button"><FiPlus /> Thêm bộ mới</button>}</div>
    </div>
