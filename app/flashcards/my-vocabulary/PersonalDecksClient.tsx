...
function CopyWordsDialog({availableWords,copyLoading,copyQuery,copySelectedIds,onClose,onCopy,onQuery,onSelection}:{availableWords:PersonalVocabulary[];copyLoading:boolean;copyQuery:string;copySelectedIds:string[];onClose:()=>void;onCopy:()=>void;onQuery:(value:string)=>void;onSelection:(ids:string[])=>void}){const visible=availableWords.filter(word=>`${word.term} ${word.kana||""} ${word.romaji||""} ${word.meaningVi}`.toLocaleLowerCase("vi").includes(copyQuery.trim().toLocaleLowerCase("vi")));return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/55 p-4 backdrop-blur-sm"><div className="max-h-[90vh] w-full max-w-3xl overflow-hidden rounded-[2rem] bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.24em] text-indigo-600">Tạo bài tổng hợp</p><h2 className="mt-2 text-3xl font-black">Thêm từ có sẵn</h2><p className="mt-2 text-sm font-semibold text-slate-500">Chọn từ trong những bộ khác của bạn để sao chép vào bộ hiện tại.</p></div><button className="grid h-10 w-10 place-items-center rounded-xl bg-slate-100" onClick={onClose} type="button"><FiX/></button></div><div className="mt-5 flex h-12 items-center rounded-2xl border px-4"><FiSearch className="text-slate-400"/><input className="ml-3 flex-1 outline-none" onChange={event=>onQuery(event.target.value)} placeholder="Tìm từ, kana, romaji hoặc nghĩa..." value={copyQuery}/></div>{copyLoading?<p className="mt-6 text-center font-bold text-slate-500">Đang tải...</p>:<><div className="mt-4 flex items-center justify-between"><button className="font-black text-indigo-700" onClick={()=>onSelection(copySelectedIds.length===visible.length?[]:visible.map(word=>word._id))} type="button">{copySelectedIds.length?"Bỏ chọn tất cả":"Chọn tất cả"}</button><span className="text-sm font-bold text-slate-500">Đã chọn {copySelectedIds.length}/{availableWords.length} từ</span></div><div className="mt-4 grid max-h-[45vh] gap-3 overflow-y-auto sm:grid-cols-2">{visible.map(word=><label className={`cursor-pointer rounded-2xl border p-4 ${copySelectedIds.includes(word._id)?"border-indigo-400 bg-indigo-50":"border-slate-200"}`} key={word._id}><div className="flex gap-3"><input checked={copySelectedIds.includes(word._id)} onChange={()=>onSelection(copySelectedIds.includes(word._id)?copySelectedIds.filter(id=>id!==word._id):[...copySelectedIds,word._id])} type="checkbox"/><div><p className="font-black">{word.term}</p><p className="text-xs font-bold text-slate-500">{[word.kana,word.romaji].filter(Boolean).join(" · ")}</p><p className="mt-2 text-sm font-bold text-teal-700">{word.meaningVi}</p></div></div></label>)}</div>{!availableWords.length&&<p className="mt-6 rounded-2xl border border-dashed p-8 text-center font-bold text-slate-500">Không còn từ nào từ các bộ khác để thêm.</p>}<button className="mt-5 h-12 w-full rounded-2xl bg-indigo-600 font-black text-white disabled:opacity-50" disabled={!copySelectedIds.length} onClick={onCopy} type="button">Thêm {copySelectedIds.length} từ vào bộ</button></>}</div></div>}
