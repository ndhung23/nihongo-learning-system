  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-10">
      ...
      <section className="mt-14 border-t border-slate-200 pt-9">
        ...
        {communityTests.length ? <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{communityTests.map((test) => <Link className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-900/[0.04] transition hover:-translate-y-1 hover:border-teal-300" href={`/flashcards/tests/${test.level.toLowerCase()}/${test.number}`} key={test._id.toString()}><span className="grid h-12 w-12 place-items-center rounded-2xl bg-teal-50 text-xl text-teal-700"><FiBookOpen /></span><div className="mt-5 flex items-start justify-between gap-3"><h3 className="text-xl font-black text-slate-950">{test.title}</h3><span className="rounded-full bg-rose-50 px-3 py-1 text-xs font-black uppercase text-rose-700">{test.level}</span></div><p className="mt-4 text-sm font-bold text-slate-500">{test.questionCount} câu · {test.sections?.listening?.length ? 3 : 2} phần</p><span className="mt-5 flex h-11 items-center justify-center rounded-2xl bg-slate-950 font-black text-white">Làm đề</span></Link>)}</div> : <div className="mt-6 rounded-3xl border border-dashed border-slate-300 bg-white/70 p-8 text-center font-bold text-slate-500">Chưa có đề thi công khai từ cộng đồng.</div>}
      </section>
    </main>
  );
}
